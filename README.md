...
**~70% overall.** Everything software-only is done and tested (245/245
...
battery, RSSI - the `request_data_stream` fix worked), and arm/disarm from
the app works against the real FC. Every remaining item below needs
...
| M2 | AI Camera / Detection | **Confirmed live again after a real, three-pass debugging saga** - a field report of zero detections turned out to be `Picamera2IMX500Camera` calling `capture_metadata()` and `capture_array()` as two separate, independently-triggered captures per frame, which could desync which frame's metadata you got from which frame's image; switched to a single `capture_request()` per frame (metadata + image from the same underlying capture, guaranteed) and detection is confirmed working live in the Android app again. Two earlier fix attempts (a missing firmware-upload wait, then a call-ordering fix for it) were real, necessary fixes along the way but not the whole story - see `docs/hardware-wiring.md` for the full sequence, each step verified against real hardware via a standalone diagnostic script before being trusted. **Tooling for the remaining acceptance-criteria gaps now built** (unit-tested against a synthetic camera/detector; each still needs to actually be *run* on the real Pi to close this milestone): `tools/benchmark_detection.py` measures FPS/p95 detection latency/mean process CPU against the plan's own thresholds (≥15 FPS, <60ms, <15% CPU) and prints a PASS/FAIL report - run it once on the bench and once outdoors with a moving human/vehicle subject to cover the plan's specific acceptance scenario. `tools/detection_regression.py` is the practical equivalent of the plan's "recorded-video regression set" for this camera's actual architecture - since the IMX500 runs inference *on the sensor* at capture time, a saved video can't be replayed through it afterward, so instead it captures a live session's detection results (class/score/bbox per frame) and analyzes frame-to-frame consistency (detection rate, longest zero-detection gap, score stability, bbox jitter) - exactly the class of check that would have caught the `capture_request()` regression above automatically instead of via a field report. Also doubles as real data for tuning `camera.score_threshold` (`hardware.yaml`, still an untuned 0.5 guess) against actual score distributions - scores as low as 0.44-0.56 were seen on real, genuine subjects during the M2 debugging saga. See `tools/README.md` for usage | 90% |
...
Test suite: `.venv/Scripts/python -m pytest -q` → 245 passed. Android: real
