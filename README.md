...
confidence threshold - now lowered to 0.35 based on real evidence; worth
re-running `tools/detection_regression.py` once more to confirm it
actually helped. Every remaining item below needs hands-on-hardware work
that hasn't happened yet - nothing is blocked on more code:
...
Test suite: `.venv/Scripts/python -m pytest -q` → 249 passed. Android: real
