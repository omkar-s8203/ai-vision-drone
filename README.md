...
| M4 | Distance Estimation | Vision (pinhole) estimator implemented + tested, now also feeding the obstacle-proximity safety check (M10). **`tools/calibrate_camera.py` has now been run against real checkerboard photos taken with the actual Pi camera**, producing real fitted intrinsics - but only on the Pi's local filesystem so far, `git log` confirms the checked-in `camera_calibration.yaml` is still the original placeholder (see `tools/README.md`); push the Pi's real file to close this out for real. Rangefinder hardware addition still open (see plan) - now the natural next decision for this milestone since the vision-only path is fully calibrated | 70% |
...
Test suite: `.venv/Scripts/python -m pytest -q` → 200 passed. Android: real
...
7. ~~Run `tools/calibrate_camera.py` against real checkerboard photos taken
   with the actual Pi camera~~ - done, producing real fitted intrinsics on
   the Pi. **Still needs one small step**: `git log` shows the checked-in
   `companion/config/camera_calibration.yaml` unchanged since it was first
   created (still the original placeholder `fx=900.0` etc.) - the real
   values only exist on the Pi's local filesystem so far. Run `git add
   companion/config/camera_calibration.yaml && git commit && git push` **on
   the Pi** to get the real calibration into version control - otherwise a
   fresh clone/pull silently reverts to the placeholder. The M4
   rangefinder hardware decision is still open (see plan) - worth deciding
   before Follow/Approach-Test get anywhere near a real flight, since the
   boundary/separation checks currently rest on the vision-only estimate
   this calibration feeds.
