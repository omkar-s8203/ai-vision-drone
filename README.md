...
| M5 | Video Streaming & Pi↔Android Comms | WebSocket control/telemetry/WebRTC-signaling channel done + tested on the Pi side; telemetry/health now actually sent. aiortc video pipeline coded but unwired-by-default and not yet exercised; GStreamer path stubbed | 45% |
...
| M13 | Testing Strategy & Simulation-Before-Flight | Synthetic target generator + mock flight controller + full end-to-end integration test all passing (61/61 tests) | 65% |
...
Test suite: `.venv/Scripts/python -m pytest -q` → 61 passed.

## What's next

1. **Verify the Android build** - open `android/` in Android Studio to resolve dependencies and fix whatever the compiler finds (most likely the `org.webrtc.*` API surface - see `android/README.md`'s known gaps).
2. **Wire the video pipeline into the orchestrator** - `AiortcVideoPipeline` exists but `build_sim_orchestrator`/`build_hardware_orchestrator` don't instantiate one yet, so `webrtc_offer` messages currently have nowhere to go.
