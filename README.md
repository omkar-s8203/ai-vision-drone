...
**~68% - camera, AI detection, video, and MAVLink are all confirmed running together on real hardware, and the Android app has been used live against the real companion session.** A Raspberry Pi 5 + AI Camera + Cube Orange flight controller are now all wired together and talking, and `companion.main` has been run in hardware mode with the FC connected live - previously camera/video and MAVLink were only proven separately; now confirmed running together, with heartbeat, telemetry, arm/disarm, and flight-mode read/set all working against the real FC (**guidance setpoints - Follow/Orbit/Approach-Test - have not been sent to the real FC yet**, only administrative commands and telemetry so far). Live AI detection also went down and came back during this hardware phase: a field report of zero detections turned into a real three-pass root-cause saga (see M2 and `docs/hardware-wiring.md`), ending in `Picamera2IMX500Camera` switching from two separate `capture_metadata()`/`capture_array()` calls to a single `capture_request()` per frame - confirmed working live again. A heatsink/fan is now installed on the Pi 5. The Android app connected to this live hardware session for real: video, drag/tap target selection, and the tracking overlay all confirmed working end-to-end on a physical phone (arm/disarm, the flight-mode dropdown, the guidance-mode buttons, recording, and the obstacle-warning banner are build-verified but not yet exercised live). Getting MAVLink working in the first place took a long debugging session whose actual root cause was a baud-rate mismatch (57600, not 921600 - the FC never actually adopted the GCS-configured baud despite the UI showing it applied) - see `docs/hardware-wiring.md` for the full story and every other real bug this hardware phase surfaced. Administrative GCS-style commands (arm/disarm, flight-mode change, local video recording), Orbit and two Dronie/Parabola smart-shot guidance modes (DJI "circle"/"QuickShot" equivalents), a full DJI-Fly-style tabbed Android redesign, and a cross-mode obstacle-proximity safety trigger have all landed and been tested (Python: fully unit-tested; Android: real `gradle assembleDebug` succeeded and the app has run on a physical device). Mounting on the aircraft and RC-override transmitter configuration haven't happened yet.

| # | Milestone | Status | % |
|---|-----------|--------|---|
| M1 | Hardware Integration & Pi Setup | Pi 5 + AI Camera + flight controller all wired and confirmed working together on a bench, with a heatsink/fan now installed for sustained AI+video thermal load. Drone mounting and flight-battery power source not done yet | 58% |
...
1. **Finish functionally verifying the Android app against the live hardware companion** -
   connect + video + drag/tap target selection + the tracking overlay are
...
3. **Send real guidance setpoints to the real FC** - `companion.main` has
   ...
   guidance controller (Follow/Orbit/Approach-Test) actually sending a
   velocity setpoint to the real FC - start with a props-off bench dry-run
   per the plan's own staged approach (M14). The dashboard for that dry-run
   now exists (`tracking_update`'s new `commanded_*`/`guidance_sent`
   fields, rendered live by Android's `GuidanceCommandPanel`) - build-
   verified but not yet watched during an actual dry-run.
