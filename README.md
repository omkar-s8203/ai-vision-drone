...
| M6 | Android Ground Station App | Builds, runs, and works on a real device: video render, drag-to-select, tracking overlay, telemetry/health panels, mode controls, abort button all confirmed live against the sim stack. Live follow-separation override and WS reconnect added; instrumented UI tests written but not yet run (no emulator here - see `android/README.md`) | 75% |
