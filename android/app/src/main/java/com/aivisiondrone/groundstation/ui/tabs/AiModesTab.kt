...
fun AiModesTab(
    ...
    LazyColumn(
        ...
        // own GPS-fix check below. A real field-reported bug: every
        // guidance mode gets silently refused by the Safety Supervisor
        // whenever the flight controller isn't actually in GUIDED yet
        // (companion/safety/supervisor.py), completely independent of
        // which target is tracked - without this hint, tapping any of
        // these buttons just silently reverted with no visible explanation
        // (now also covered reactively by AlertEvent.MODE_REJECTED_FC_NOT_GUIDED
        // in MainViewModel).
