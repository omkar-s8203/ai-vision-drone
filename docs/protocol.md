...
| `telemetry` | Pi → Android | `{fc_mode, armed, lat, lon, alt_m, groundspeed_mps, battery_voltage_v, battery_remaining_pct, fence_enabled, fence_breached, satellites_visible, gps_fix_type}` (mirrors `MavlinkBridge.telemetry`) - `fence_enabled`/`fence_breached` come from the FC's real `SYS_STATUS` geofence bit, so the operator can see geofence status live rather than only learning about it if an Approach-Test abort happens to reference it in `guidance_reason`. `satellites_visible`/`gps_fix_type` come from a real `GPS_RAW_INT` message (`satellites_visible` is `null` when the FC reports the standard "unknown" sentinel of 255, not zero satellites) - a real bug found from a UI review: the Android HUD previously showed a hardcoded fake `"12"` for satellite count since nothing populated a real value; `gps_fix_type` follows the standard `MAV_GPS_FIX_TYPE` enum (0/1 = no fix, 2 = 2D, 3+ = 3D or better) and is the authoritative signal for the GPS "FIX"/"NO FIX" indicator, which previously only inferred fix status from `lat` being non-null - a much less precise proxy, since a stale/degraded fix can still report a non-null last-known position |
